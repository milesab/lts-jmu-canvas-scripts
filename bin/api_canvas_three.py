...
def job_status(job_arg, endpoint_arg):
    ...
    if endpoint_arg == 'import':
        endpoint = 'accounts/%s/sis_imports/%s.json?access_token=%s' % (
            account_id, job_id, access_token)
    elif endpoint_arg == 'export':
        endpoint = 'accounts/%s/reports/provisioning_csv/%s?access_token=%s' % (
            account_id, job_id, access_token)
    elif endpoint_arg == 'last_user_access_csv':
        endpoint = 'accounts/%s/reports/last_user_access_csv/%s?access_token=%s' % (
            account_id, job_id, access_token)
    else:
        endpoint = endpoint_arg
    if job_id:
        response = urllib.request.urlopen(base_url + endpoint).read().strip()
...
def import_submit(import_file, import_id_file):
    if os.path.isfile(import_file):
        ...
        url = f'{base_url}accounts/{account_id}/sis_imports.json'
...
def export_submit(export_id_file):
    ...
    params = {"parameters": {'terms': '1', 'courses': '1',
                             'sections': '1', 'enrollments': '1', 'users': '1', 'xlist': '1'}}
...
def report_download(file_url, report_file):
    report_request = urllib.request.Request(
        file_url, None, {'Authorization': 'Bearer %s' % access_token})
    report_data = urllib.request.urlopen(file_url)
...
def get_courseinfo(course_id):
    ...
    courseinfo_request = urllib.request.Request(
        courseinfo_endpoint, None, {'Authorization': 'Bearer %s' % access_token})
    courseinfo_response = urllib.request.urlopen(
        courseinfo_request).read().strip()
...
def get_students(course_id):
    students = []
    students_endpoint = base_url + \
        'courses/%s/users?enrollment_type=student&per_page=100' % course_id
    ...
    while more_pages:
        ...
        if 'next' in list(student_request.links.keys()):
...
def get_sections(course_id):
    ...
    section_request = urllib.request.Request(
        section_endpoint, None, {'Authorization': 'Bearer %s' % access_token})
    section_response = urllib.request.urlopen(section_request).read().strip()
...
def get_scores(student_data, course_id):
    ...
    for group in chunks(student_ids, 10):
        ...
        submission_params = urllib.parse.urlencode(submission_params)
        req = urllib.request.Request(submissions_endpoint + "?" + submission_params, None, {
                                     'Authorization': 'Bearer %s' % access_token})
        score_response = urllib.request.urlopen(req).read().strip()
