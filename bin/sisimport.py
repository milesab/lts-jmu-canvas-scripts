...
import_file = config['local']['easel_home'] + 'data/temp/sisimport.zip'
import_id_file = config['local']['easel_home'] + 'data/temp/sisimport_id.txt'
...
def enrollment_changes():
    matches = []
    enrollment_changes = api_local.read_csv(config['local']['easel_home'] + 'data/add_enroll/enrollment_changes.csv', 'course_id')
    ...
    for line in fileinput.input(config['local']['import_dir'] + 'enrollments.csv', inplace=1):
        if line in matches:
...
if __name__ == '__main__':
    ...
    enrollment_changes()
    api_canvas.import_zip(import_file,config['local']['import_dir'])
